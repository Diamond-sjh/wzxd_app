<template>
	<view class="homeContainer">
		...
		<uni-card :title="item.routeLine" thumbnail="" :extra="item.routeSection" v-for="item in dataList" :key=item.id note="true">
			<view>隧道名称: {{item.chunnel}}</view>
			...
			<view>检测项目: {{item.checkItem}}</view>
			<view>是否故障: {{item.isfault==1?'故障':'正常'}}</view>
			<template v-slot:footer >
				<button type="primary" size="mini" class="footerBtn" @click="jumpToPage('addVirse',item)">添加病害</button>
				<button type="primary" size="mini" class="footerBtn" @click="jumpToPage('update',item)">修改/新增</button>
				<button type="warn" size="mini" class="footerBtn" @click="jumpToPage('del',item)">删除</button>
			</template>
		</uni-card>
