  ...
  data() {
	...
	methods: {
		...
		getServerData(stationVal) {
			...
			let params = {
				dataBase:stationVal.databases,
				tabId:stationVal.tabId,
				tableName:stationVal.tableName
			}
			this.$http.earthquake(params).then(res => {
				if(res.code == 200){
					res.data.forEach(val=>{
						data2.push(val.collectTime.split('.')[0].replace(' ', '\n'))
						if(val.accelerationX){
							data3.push(val.accelerationX)
							return
						}
						if(val.accelerationY){
							data3.push(val.accelerationY)
							return
						}
						if(val.accelerationZ){
							data3.push(val.accelerationZ)
							return
						}
					})
					this.ringOpts = {
